...
const penner = (() => {
	...
	const baseEasing = ['Quad', 'Cubic', 'Quart', 'Quint', 'Expo'];

	baseEasing.forEach((name, i) => {
		functionEasing[name] = () => (t) => Math.pow(t, i + 2);
	});

	Object.keys(functionEasing).forEach((name) => {
		const easeIn = functionEasing[name];
...
function parseEasing(easing, duration) {
...
function getUnit(val) {
	const split = /[+-]?\d*\.?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?(%|px|pt|em|rem|in|cm|mm|ex|ch|pc|vw|vh|vmin|vmax|deg|rad|turn)?$/.exec(
		val,
	);
...
function getAnimatable(targets) {
...
function normalizeTweens(prop, animatable) {
	...
	return prop.tweens.map((t) => {
		...
		tween.easing = parseEasing(tween.easing, tween.duration);
...
function setTargetsValue(targets, properties) {
	const animatable = getAnimatable(targets);
...
function getAnimations(animatable, properties) {
	return filterArray(
		flattenArray(
			animatable.map((animatable) => {
...
function createNewInstance(params) {
	...
	const animatable = getAnimatable(params.targets);
	const animations = getAnimations(animatable, properties);
		...
		if (activeInstancesLength) {
			...
			while (i < activeInstancesLength) {
				...
				if (!activeInstance.paused) {
					...
					const instanceIndex = activeInstances.indexOf(
						activeInstance,
					);
					if (instanceIndex > -1) {
...
function stagger(val, params = {}) {
	...
	const easing = params.easing ? parseEasing(params.easing) : null;
