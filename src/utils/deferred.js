import { isDeferred, isDefined, isPromise } from './type';
		...
		['done', 'fail'].forEach(
			function (method) {
				...
				this[method](function () {
					if (!callback) {
						result[method === 'done' ? 'resolve' : 'reject'].apply(
							this,
							arguments
						);
						return;
					...
					if (isDeferred(callbackResult)) {
						...
						result.resolve.apply(
							this,
							isDefined(callbackResult)
								? [callbackResult]
								: arguments
						);
					}
	...
	if (isDeferred(promise)) {
		...
		promise.then(
			function () {
				d.resolveWith.apply(
					d,
					[context].concat([[].slice.call(arguments)])
				);
			},
			function () {
				d.rejectWith.apply(
