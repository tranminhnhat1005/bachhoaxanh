...
const convertMillisecondsToDateUnits = function (value) {
	...
	for (i = dateUnitIntervals.length - 1; i >= 0; i--) {
		...
		if (dateUnitCount > 0) {
			...
			value -= convertDateUnitToMilliseconds(
				dateUnitInterval,
				dateUnitCount
			);
		}
...
const dateToMilliseconds = function (tickInterval) {
	...
	if (isObject(tickInterval)) {
		each(tickInterval, function (key, value) {
			milliseconds += convertDateUnitToMilliseconds(
	...
	if (isObject(tickInterval)) {
		each(tickInterval, function (key, value) {
			for (i = 0; i < dateUnitIntervals.length; i++) {
				if (
					value &&
					(key === dateUnitIntervals[i] + 's' ||
						key === dateUnitIntervals[i]) &&
					maxInterval < i
				) {
					maxInterval = i;
				}
...
function correctDateWithUnitBeginning(
	...
	if (
		withCorrection &&
		dateUnitInterval !== 'hour' &&
		dateUnitInterval !== 'minute' &&
		dateUnitInterval !== 'second'
	) {
...
function addDateInterval(value, interval, dir) {
	...
	const intervalObject = isString(interval)
		? getDateIntervalByString(interval.toLowerCase())
		: isNumeric(interval)
		? convertMillisecondsToDateUnits(interval)
		: interval;
...
const addInterval = function (value, interval, isNegative) {
	...
	return isDate(value)
		? addDateInterval(value, interval, dir)
		: adjust(value + interval * dir, interval);
};
...
const getViewFirstCellDate = function (viewType, date) {
	...
	if (viewType === 'decade') {
		return createDateWithFullYear(
			getFirstYearInDecade(date),
			date.getMonth(),
			date.getDate()
		);
	}
...
const getViewLastCellDate = function (viewType, date) {
	...
	if (viewType === 'century') {
		return createDateWithFullYear(
			getFirstDecadeInCentury(date) + 90,
			date.getMonth(),
			date.getDate()
		);
	}
...
const getViewMinBoundaryDate = function (viewType, date) {
	const resultDate = createDateWithFullYear(
		date.getFullYear(),
		date.getMonth(),
		1
	);
...
function sameDate(date1, date2) {
	return (
		sameMonthAndYear(date1, date2) && date1.getDate() === date2.getDate()
	);
}
...
const normalizeDateByWeek = function (date, currentDate) {
	const differenceInDays = dateUtils.getDatesInterval(
		date,
		currentDate,
		'day'
	);
...
const intervalsOverlap = function (options) {
	const { firstMin, firstMax, secondMin, secondMax } = options;

	return (
		(firstMin <= secondMin && secondMin <= firstMax) ||
...
const roundDateByStartDayHour = function (date, startDayHour) {
	...
	if (
		(date.getHours() === startTime.hours &&
			date.getMinutes() < startTime.minutes) ||
		date.getHours() < startTime.hours
	) {
