import { getWindow } from './device/window';
import { isWindow, isString, isNumeric } from './type';
...
const getElementBoxParams = function (name, elementStyles) {
	...
	return {
		padding: getSizeByStyles(elementStyles, [
			'padding' + beforeName,
			'padding' + afterName
		]),
		border: getSizeByStyles(elementStyles, [
			'border' + beforeName + 'Width',
			'border' + afterName + 'Width'
		]),
		margin: getSizeByStyles(elementStyles, [
			'margin' + beforeName,
			'margin' + afterName
		])
	};
...
const getBoxSizingOffset = function (name, elementStyles, boxParams) {
	...
	if (
		elementStyles.boxSizing === 'border-box' &&
		size.length &&
		size[size.length - 1] !== '%'
	) {
...
const parseHeight = function (value, container) {
	if (value.indexOf('px') > 0) {
		...
		value =
			(parseInt(value.replace('%', '')) * getContainerHeight(container)) /
			100;
	} else if (!isNaN(value)) {
...
const getVerticalOffsets = function (element, withMargins) {
	...
	const boxParams = getElementBoxParams(
		'height',
