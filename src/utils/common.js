...
import { when, Deferred } from './deferred';
...
export const executeAsync = function (action, context /* , internal */) {
	...
	const timerId = (arguments[2] || setTimeout)(
		callback,
...
const deferExecute = function (name, func, deferred) {
	if (executingName && executingName !== name) {
		...
		if (!executingName && delayedFuncs.length) {
			(delayedNames.shift() === 'render' ? deferRender : deferUpdate)(
				delayedFuncs.shift(),
				delayedDeferreds.shift()
			);
		}
		return result || when();
	}
...
export const pairToObject = function (raw, preventRound) {
	...
	let h = preventRound
		? parseFloat(pair && pair[0])
		: parseInt(pair && pair[0], 10);
	let v = preventRound
		? parseFloat(pair && pair[1])
		: parseInt(pair && pair[1], 10);
