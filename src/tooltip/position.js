 ...
 */
const parseAlignMode = direction => {
 ...
 */
const getUpDownPosition = (tip, target, state, direction, alignMode, props) => {
  let left = -10000000;
  let top;
  ...
  if (tip) {
    ...
    if (direction === 'up') {
      top =
        targetRect.top + getScrollTop () - (tip.offsetHeight + arrowSpacing);
    } else {
 ...
 */
const getArrowStyles = (target, tip, direction, state, props) => {
  ...
  switch (direction) {
    case 'right':
      ...
      positionStyles.top = state.showTip && tip
        ? targetRect.top + scrollTop + halfTargetHeight - props.arrowSize
        : '-10000000px';
      positionStyles.left =
    ...
    case 'left':
      ...
      positionStyles.top = state.showTip && tip
        ? targetRect.top + scrollTop + halfTargetHeight - props.arrowSize
        : '-10000000px';
      positionStyles.left = targetRect.left + scrollLeft - arrowSpacing;
      break;
    case 'up':
...
export default function positions (
  ...
  if (tip) {
    ...
    const spacer = tip.style.width ? 0 : 5;
