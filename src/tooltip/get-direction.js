...
const checkTargetSufficientlyVisible = (target, tip, props) => {
  const targetRect = target.getBoundingClientRect ();
  ...
  if (target.offsetHeight > tip.offsetHeight) {
    ...
    const targetCenterToBottomOfWindow =
      halfTargetHeight - bottomOverhangAmount;
    const targetCenterToTopOfWindow = halfTargetHeight - topOverhangAmount;

    return (
      targetCenterToBottomOfWindow >= arrowClearance &&
      targetCenterToTopOfWindow >= arrowClearance
    );
  }
...
const checkForArrowOverhang = (props, arrowStyles, bodyPadding) => {
  const scrollLeft = getScrollLeft ();
  const hasLeftClearance =
    arrowStyles.positionStyles.left - scrollLeft > bodyPadding;
  const hasRightClearance =
    arrowStyles.positionStyles.left + props.arrowSize * 2 <
...
export default function getDirection (
  ...
  switch (currentDirection) {
    case 'right':
      if (
        !checkLeftRightWidthSufficient (
          tip,
          target,
          arrowSpacing,
          bodyPadding
        ) ||
        !checkTargetSufficientlyVisible (target, tip, props)
      ) {
        return getDirection (
          'up',
          tip,
    ...
    case 'up':
      if (
        !recursive &&
        arrowStyles &&
        checkForArrowOverhang (props, arrowStyles, bodyPadding)
      ) {
        return getDirection (
          'left',
          tip,
