...
const PATTERN_REGEXPS = {
	...
	Q: function (count, dateParts) {
		if (count > 2) {
			return dateParts
				.getQuarterNames(FORMAT_TYPES[count], 'format')
				.join('|');
		}
		return '0?[1-4]';
	},
	...
	a: function (count, dateParts) {
		return dateParts
			.getPeriodNames(FORMAT_TYPES[count < 3 ? 3 : count], 'format')
			.join('|');
	},
	d: function (count) {
		...
		return ['format', 'standalone']
			.map(function (type) {
				return Object.keys(FORMAT_TYPES).map(function (count) {
					const monthNames = dateParts.getMonthNames(
						FORMAT_TYPES[count],
						type
					);
...
const PATTERN_PARSERS = {
	...
	E: function (text, count, dateParts) {
		const dayNames = dateParts.getDayNames(
			FORMAT_TYPES[count < 3 ? 3 : count],
			'format'
		);
		return caseInsensitiveIndexOf(dayNames, text);
	},
	a: function (text, count, dateParts) {
		const periodNames = dateParts.getPeriodNames(
			FORMAT_TYPES[count < 3 ? 3 : count],
			'format'
		);
...
export const getParser = function (format, dateParts) {
	...
	return function (text) {
		...
		if (regExpResult) {
			...
			const orderedFormatPatterns = getOrderedFormatPatterns(
				formatPatterns
			);
			...
			orderedFormatPatterns.forEach(function (pattern, index) {
				if (
					!pattern ||
					(index < ORDERED_PATTERNS.length && index > maxPatternIndex)
				) {
					return;
				}
