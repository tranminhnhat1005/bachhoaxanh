...
const checkDigit = function (char) {
	const code =
		char && numberLocalization.convertDigits(char, false).charCodeAt(0);
...
const checkPatternContinue = function (text, patterns, index, isDigit) {
	...
	if (!isDigit) {
		if (
			char === '.' ||
			(char === ' ' && text.slice(index - 1, index + 3) === AM_PM_PATTERN)
		) {
			return true;
		}
	...
	const isDigitChanged =
		isDigit && patterns.some((pattern) => text[index] !== pattern[index]);
	return (
		FORMAT_SEPARATORS.indexOf(char) < 0 &&
		isDigit === checkDigit(char) &&
		(!isDigit || isDigitChanged)
	);
};
...
const getDifference = function (
	...
	for (i = 0; i < defaultPattern.length; i++) {
		if (
			processedIndexes.indexOf(i) < 0 &&
			patterns.filter(patternsFilter).length
		) {
			...
			do {
				...
				defaultPattern[i] &&
				checkPatternContinue(defaultPattern, patterns, i, isDigit)
			);
			break;
		}
	}
	...
	if (
		result.length === 1 &&
		(defaultPattern[processedIndexes[0] - 1] === '0' ||
			defaultPattern[processedIndexes[0] - 1] === '٠')
	) {
...
const replaceCharsCore = function (pattern, indexes, char, patternPositions) {
	...
	const patternIndex =
		baseCharIndex < patternPositions.length
			? patternPositions[baseCharIndex]
			: baseCharIndex;

	indexes.forEach(function (_, index) {
		pattern =
			pattern.substr(0, patternIndex + index) +
...
const replaceChars = function (pattern, indexes, char, patternPositions) {
	...
	if (!checkDigit(pattern[indexes[0]] || '0')) {
		...
		while (indexes.length > letterCount) {
			...
			pattern =
				pattern.substr(0, patternIndex) +
				pattern.substr(patternIndex + 1);
		}

		(index = indexes[indexes.length - 1] + 1),
			(patternIndex =
				index < patternPositions.length
					? patternPositions[index]
					: index);
...
const escapeChars = function (
	...
	const escapeIndexes = defaultPattern.split('').map(function (char, index) {
		if (
			processedIndexes.indexOf(index) < 0 &&
			(char.match(ESCAPE_CHARS_REGEXP) || char === "'")
		) {
	...
	pattern = pattern
		...
		.map(function (char, index) {
			...
			const isPrevCharEscaped =
				index > 0 && escapeIndexes.indexOf(index - 1) >= 0;
...
export const getFormat = function (formatter) {
	...
	const defaultPattern = formatValue(
		new Date(2009, 8, 8, 6, 5, 4),
		formatter
	);
	...
	datePatterns.forEach(function (test) {
		...
		const pattern =
			test.pattern === 'M' && !replacedPatterns['d'] ? 'L' : test.pattern;
