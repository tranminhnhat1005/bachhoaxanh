import { fitIntoRange } from '../../utils/math';
...
function formatNumberPart(format, valueString) {
	return format
		.split(ESCAPING_CHAR)
		.map(function (formatPart, escapeIndex) {
			...
			return isEscape
				? formatPart
				: formatPart.replace(/[,#0]+/, valueString);
		})
		.join('');
}
...
export function getFormatter(format, config) {
	...
	return function (value) {
		...
		const maxFloatPrecision =
			minFloatPrecision + getNonRequiredDigitCount(floatFormatParts[1]);
		const minIntegerPrecision = getRequiredDigitCount(floatFormatParts[0]);
		const maxIntegerPrecision =
			getNonRequiredDigitCount(floatFormatParts[0]) ||
			config.unlimitedIntegerDigits
				? undefined
				: minIntegerPrecision;
		...
		const valueParts = toFixed(
			value,
			floatPrecision < 0 ? 0 : floatPrecision
		).split('.');
		...
		const integerString = reverseString(
			formatNumberPart(
				reverseString(floatFormatParts[0]),
				valueIntegerPart
			)
		);
		const floatString = maxFloatPrecision
			? formatNumberPart(floatFormatParts[1], valueFloatPart)
			: '';

		const result =
			integerString +
			(floatString.match(/\d/) ? config.decimalSeparator : '') +
			floatString;
...
function getFormatByValueText(valueText, formatter, isPercent, isNegative) {
	...
	const valueTextWithModifiedFloat =
		valueTextParts[0] + '.3' + valueTextParts[1].slice(1);
	const valueWithModifiedFloat = parseValue(
		valueTextWithModifiedFloat,
		isPercent,
		isNegative
	);
	const decimalSeparatorIndex =
		formatter(valueWithModifiedFloat).indexOf('3') - 1;

	format = format.replace(/(\d)\D(\d)/g, '$1,$2');

	if (decimalSeparatorIndex >= 0) {
		format =
			format.slice(0, decimalSeparatorIndex) +
			'.' +
			format.slice(decimalSeparatorIndex + 1);
	}
...
export function getFormat(formatter) {
	...
	const positiveFormat = getFormatByValueText(
		valueText,
		formatter,
		isPercent,
		false
	);
	const negativeFormat = getFormatByValueText(
		valueText,
		formatter,
		isPercent,
		true
	...
	return negativeFormat === '-' + positiveFormat
		? positiveFormat
