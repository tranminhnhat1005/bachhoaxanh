import dependencyInjector from '../utils/dependency_injector';
import { inArray, find } from '../utils/array';
...
const numberLocalization = dependencyInjector({
	...
	_calculateNumberPower: function (value, base, minPower, maxPower) {
		...
		if (number > 1) {
			while (
				number &&
				number >= base &&
				(maxPower === undefined || power < maxPower)
			) {
				power++;
				number = number / base;
			}
	...
	_formatNumber: function (value, formatObject, formatConfig) {
		if (formatObject.power === 'auto') {
			formatObject.power = this._calculateNumberPower(
				value,
				1000,
				0,
		...
		const powerPostfix =
			this.defaultLargeNumberFormatPostfixes[formatObject.power] || '';

		let result = this._formatNumberCore(
			value,
			formatObject.formatType,
			formatConfig
		);
	...
	_formatNumberExponential: function (value, formatConfig) {
		...
		return (
			this._formatNumberCore(number, 'fixedpoint', formatConfig) +
			'E' +
			powString
		);
	},
	...
	_addGroupSeparators: function (value) {
		...
		return (
			...
			) + (parts[1] ? config().decimalSeparator + parts[1] : '')
		);
	},
	...
	convertDigits: function (value, toStandard) {
		...
		return value.replace(regExp, (char) => {
			return String.fromCharCode(
				char.charCodeAt(0) +
					(toFirstDigit.charCodeAt(0) - fromFirstDigit.charCodeAt(0))
			);
		});
	},
	...
	getNegativeEtalonRegExp: function (format) {
		...
		const digitalRegExp = new RegExp(
			'[0-9' +
				escapeRegExp(
					separators.decimalSeparator + separators.thousandsSeparator
				) +
				']+',
			'g'
		);
		...
		let negativeEtalon = this.format(-1, format).replace(
			digitalRegExp,
			'1'
		);
	...
	format: function (value, format) {
		...
		if (!numberConfig) {
			...
			formatterConfig.unlimitedIntegerDigits =
				format.unlimitedIntegerDigits;
			return this.convertDigits(
				getFormatter(format.type, formatterConfig)(value)
			);
		}
	...
	parse: function (text, format) {
		...
		const regExp = new RegExp(
			'[^0-9' + escapeRegExp(decimalSeparator) + ']',
			'g'
		);
		...
		if (power) {
			if (power === 'auto') {
				...
				if (match) {
					power = find(
						Object.keys(LargeNumberFormatPostfixes),
						(power) =>
							LargeNumberFormatPostfixes[power] === match[1]
					);
				}
			}
	...
	_calcSignificantDigits: function (text) {
		...
		if (fractional) {
			result += calcDigitsAfterLeadingZeros(
				fractional.split('').reverse()
			);
		}
		return result;
	}
