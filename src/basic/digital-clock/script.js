...
class IosSelector {
	constructor(options) {
		...
		this.radius =
			this.itemHeight / Math.tan((this.itemAngle * Math.PI) / 180); // 圆环半径
	...
	_init() {
		...
		if (this.source.length) {
			this.value =
				this.value !== null ? this.value : this.source[0].value;
	...
	_touchmove(e, touchData) {
		...
		if (this.type === 'normal') {
			if (moveToScroll < 0) {
				...
				moveToScroll =
					this.source.length +
					(moveToScroll - this.source.length) * 0.3;
			}
	...
	_touchend(e, touchData) {
		...
		if (touchData.yArr.length === 1) {
			...
			// 计算速度
			v =
				(((startY - endY) / this.itemHeight) * 1000) /
				(endTime - startTime);
			let sign = v > 0 ? 1 : -1;
    ...
    `;
		...
		if (this.options.type === 'infinite') {
			...
			// 高亮头尾
			highListHTML =
				`<li class="highlight-item" style="height: ${
					this.itemHeight
				}px;">
		...
		this.elems.circleItems = this.elems.el.querySelectorAll(
			'.select-option',
		);

		this.elems.highlight = this.elems.el.querySelector('.highlight');
		this.elems.highlightList = this.elems.el.querySelector(
			'.highlight-list',
		);
			...
			let tick = () => {
				...
				if (pass < t) {
					this.scroll = this._moveTo(
						initScroll +
							easing[easingName](pass / t) * totalScrollLen,
					);
	...
	select(value) {
		for (let i = 0; i < this.source.length; i++) {
			if (this.source[i].value === value) {
				...
				let finalScroll = i;
				let t = Math.sqrt(
					Math.abs((finalScroll - initScroll) / this.a),
				);
	...
	destroy() {
		...
		// document 事件解绑
		for (let eventName in this.events) {
			this.elems.el.removeEventListener(
				'eventName',
