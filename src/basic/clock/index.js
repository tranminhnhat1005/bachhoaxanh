...
import ClockIcon from '../../icon/clock/clock';
import CancelIcon from '../../icon/cancel/cancel';
...
const cutPX = (x) => {
	// return x.slice(x.length - 2, x.length) === 'px' ? x.slice(0, -2) : x + 'px';
...
const CLock = forwardRef((props, ref) => {
	...
	const BoxMain = styled.div`
		...
		:hover:not(.focused) {
			...
			${'#' + IDs.input}::placeholder, ${'#' + IDs.icon} ,${'#' +
			IDs.timeType} {
				color: #111d5e;
			}
			&::before {
	...
	const onChangeMinuteFace = (e) => {
		face = 1;
		const minuteFace = getElById(IDs.clockMinuteFace);
		if (minuteFace.style.transform !== 'scale(1) translateY(-100%)') {
			const hourFace = getElById(IDs.clockHourFace);
			const hourBtn = getElById(IDs.hourBtn);
			onRotateMinuteNeedle(e.target.innerText);
			hourBtn.classList.remove('active');
			hourFace.style.transform = 'scale(0)';
			minuteFace.style.transform = 'scale(1) translateY(-100%)';
