...
export class TimePicker extends React.Component {
  ...
  get autoSwitchToMinutes () {
    return this.nativeElement
      ? this.nativeElement.autoSwitchToMinutes
      : undefined;
  }
  ...
  get localizeFormatFunction () {
    return this.nativeElement
      ? this.nativeElement.localizeFormatFunction
      : undefined;
  }
  ...
  /** Sets or gets the name attribute for the element. Name is used when submitting HTML forms.
	*	Property type: string
	*/
  get name () {
    ...
    for (let prop in props) {
      if (prop === 'class') {
        ...
        for (let className in classNames) {
          if (
            !that.nativeElement.classList.contains (classNames[className]) &&
  ...
  componentWillUnmount () {
    ...
    for (let i = 0; i < that.events.length; i++) {
      ...
      that.nativeElement.removeEventListener (
