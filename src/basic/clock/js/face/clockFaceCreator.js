...
export default class ClockFaceCreator {
	...
	create(clockItems, innerClockItems, outerClockItems, face) {
		ClockFaceCreator.doCreate(clockItems, this.clockElem, (span) =>
			span.classList.add(css.item),
		);
	...
	static doCalculateSize(middleX, middleY, radius, items) {
		...
		for (let i = 0; i < items.length; i++) {
			...
			item.style.left =
				middleX + Math.sin(angle) * radius - itemWidth / 2 + 'px';
			item.style.bottom =
				middleY + Math.cos(angle) * radius - itemHeight / 2 + 'px';
		}
	}
}
